...
with open(out_fn.replace(".nii.gz", "_volumes.txt"), "w") as h:
    h.write("label,side,region_name,vol,weighted_vol\n")
    for idx in sorted(name_d.keys()):
        h.write("%d,L,%s,%.2f,%.2f\n" % (idx, name_d[idx], label_sum[idx], label_weightedsum[idx]))
    for idx in sorted(name_d.keys()):
        h.write("%d,R,%s,%.2f,%.2f\n" % (idx, name_d[idx], label_sum[idx+100], label_weightedsum[idx+100]))
