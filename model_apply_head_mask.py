...
OUTPUT_RES64 = False # almost unused
OUTPUT_NATIVE = True
OUTPUT_DEBUG = False
...
for fname in sys.argv[1:]:
    ...
    try:
        ...
        outfilename = fname.replace(".img", ".nii").replace(".hdr", ".nii").replace(".mnc", ".nii").replace(".mgz", ".nii").replace(".nii.gz", ".nii").replace(".nii", "_tiv.nii.gz")
...
## Head priors
    ...
    brainmask_cc = torch.tensor(output).to(device)
...
## MNI affine
    ...
    if 0: # enforce rigid-space for hippocampus processing
        MO = np.zeros((4,4), np.float32)
        MO = M.copy()
