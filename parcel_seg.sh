...
[ "${0:0:1}" = . ] && scriptpath=${PWD}/${scriptpath}
...
Options: -c N : extract label N and uses that for input, useful if the input is a label map.
...
if [ $NOREG ]; then
    ...
    antsRegistrationSyNQuick.sh -m res64_${a}.nii -f ${scriptpath}/templates/TPM_c1.nii.gz -t a -o aff_${a} -n 2
...
for atlas in $atlas_list; do
    ...
    python $scriptpath/post_assemble.py ${a1} Lout_${a}_${atlas}_filled.nii Rout_${a}_${atlas}_filled.nii ${a}_labelled_${atlas}.nii.gz ${seg_threshold:-".5"}
done
