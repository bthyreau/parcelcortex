...
with open(out_fn.replace(".nii.gz", "_volume_metric.txt"), "w") as h:
    h.write("label,side,region_name,vol,weighted_vol\n")
    for idx in sorted(name_d.keys()):
        h.write("%d,L,%s,%.2f,%.2f\n" % (idx, name_d[idx], label_sumL[idx], label_weightedsumL[idx]))
    for idx in sorted(name_d.keys()):
        h.write("%d,R,%s,%.2f,%.2f\n" % (idx, name_d[idx], label_sumR[idx], label_weightedsumR[idx]))
