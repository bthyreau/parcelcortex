...
torch.backends.cudnn.benchmark=True # False is faster on GTX1080TI on pytorch 1.2 non pip
...
for fnamel in sys.argv[1:]:
    try:
        ...
        print("************** Failure with %s ****************** " % fnamel)
    print("%s done (%2.2fs)" % (fnamel, time.time()-T))
    T = time.time()
